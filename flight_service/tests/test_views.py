...
from ..serializers import CrewSerializer, FlightSerializer, OrderSerializer
...
class CrewViewSetTest(TestCase):
    def setUp(self):
        ...
        self.view = CrewViewSet.as_view({'get': 'list', 'post': 'create', 'retrieve': 'retrieve'})
...
class FlightViewSetTest(TestCase):
    def setUp(self):
        self.factory = APIRequestFactory()
        self.view = FlightViewSet.as_view({'get': 'list', 'post': 'create', 'retrieve': 'retrieve'})
        self.user = get_user_model().objects.create_user("admin@user.com", "password", is_staff=True)
        self.source_airport = Airport.objects.create(icao="KJFK", iata="JFK",
        ...
        self.airplane = Airplane.objects.create(name="Test Airplane", rows=5, seats_in_row=4, airplane_type=self.airplane_type)
